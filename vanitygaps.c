...
/* Public declarations shared with config.h via vanitygaps.h */
...
getfacts(Monitor *m, int msize, int ssize, float *mf, float *sf, int *mr, int *sr)
{
	...
	*mr = msize - mtotal;  // remainder after a cfacts master split
	*sr = ssize - stotal;  // remainder after a cfacts stack split
}
...
deck(Monitor *m)
{
	...
	if (n - m->nmaster > 0) /* override layout symbol */
		snprintf(m->ltsymbol, sizeof m->ltsymbol, "D %d", n - m->nmaster);  // NOLINT(build/include_what_you_use)
