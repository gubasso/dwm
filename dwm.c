	...
	if (XrmGetResource(xrdb, R, NULL, &type, &value) == True) { \
		if (value.addr != NULL && strnlen(value.addr, 8) == 7 && value.addr[0] == '#') { \
			...
			for (; i <= 6; i++) { \
				if (value.addr[i] < '0') \
					break; \
				if (value.addr[i] > '9' && value.addr[i] < 'A') \
					break; \
				if (value.addr[i] > 'F' && value.addr[i] < 'a') \
					break; \
				if (value.addr[i] > 'f') \
					break; \
			} \
...
static void setscratchpadsize(Client *c);
...
static char cstext[1024]; /* center status text */
static int statusbarx;    /* x where right status begins, set by drawstatusbar */
static int screen;
...
setscratchpadsize(Client *c)
{
	...
	for (i = 0; i < LENGTH(scratchpads); i++) {
		if (!(c->tags & SPTAG(i)))
			continue;
		if (scratchpads[i].width_pct > 0) {
			w = c->mon->ww * scratchpads[i].width_pct / 100;
			c->w = MAX(1, w - 2 * c->bw);
		}
		if (scratchpads[i].height_pct > 0) {
			h = c->mon->wh * scratchpads[i].height_pct / 100;
			c->h = MAX(1, h - 2 * c->bw);
		}
		break;
	}
		...
		if (text[i] == '^' && !iscode) {
			...
			while (text[++i] && text[i] != '^') {
				if (text[i] == 'c') {
					...
					hex[7] = '\0';
					drw_clr_create(drw, &drw->scheme[ColFg], hex, baralpha);
					i += 7;
				} else if (text[i] == 'b') {
					char hex[8];
					if (strlen(text + i + 1) < 7)
					...
					memcpy(hex, text + i + 1, 7);
					hex[7] = '\0';
					drw_clr_create(drw, &drw->scheme[ColBg], hex, baralpha);
					i += 7;
				} else if (text[i] == 'C') {
					char *end;
					long c;
					if (text[i + 1] < '0' || text[i + 1] > '9')
						continue;
					errno = 0;
					c = strtol(text + i + 1, &end, 10);
					if (end == text + i + 1 || errno == ERANGE)
						continue;
					if (c < 0)
						c = 0;
					if (c > 15)
						c = 15;
					drw_clr_create(drw, &drw->scheme[ColFg], termcolor[c], baralpha);
					i += (int)(end - (text + i + 1));
				} else if (text[i] == 'B') {
					char *end;
					long c;
					...
					if (c > 15)
						c = 15;
					drw_clr_create(drw, &drw->scheme[ColBg], termcolor[c], baralpha);
					i += (int)(end - (text + i + 1));
				} else if (text[i] == 'd') {
					drw->scheme[ColFg] = scheme[SchemeNorm][ColFg];
					drw->scheme[ColBg] = scheme[SchemeNorm][ColBg];
...
grabkeys(void)
{
	updatenumlockmask();
	{
		...
		if (currentkey > 0)
			XGrabKey(dpy, XKeysymToKeycode(dpy, XK_Escape), AnyModifier,
